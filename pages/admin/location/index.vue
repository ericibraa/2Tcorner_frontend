        ...
        <div>
            <v-card>
                <v-card-title style="background: #eceff1">
                    <v-btn color="light-blue" dark :to="'/admin/location/tambah'">Tambah Lokasi</v-btn>
                    <v-spacer></v-spacer>
                ...
                <v-card-text class="pa-5">
                    <v-data-table :headers="headers" :items="locations" :items-per-page="5">
...
export default {
    ...
    methods: {
        editItem(item) {
            this.$router.push("/admin/location/" + item._id)
        },
        deleteItem(item) {
        ...
        async deleteItemConfirm(id) {
            try {
                await this.$axios.$delete(this.$config.api + "/location/" + id)
                this.$nuxt.refresh();
