      ...
      <v-card>
        ...
        <v-card-text class="pa-5">
          <v-row>
            ...
            <v-col cols="4">
              <v-select :items="merks" label="Merek Kendaraan" outlined item-text="name" hide-details
                v-model="form.merk"></v-select>
            </v-col>
            <v-col cols="4">
              <v-select :items="types" label="Tipe Kendaraan" outlined item-text="name" hide-details
...
export default {
  ...
  methods: {
    ...
    async submitProduct() {
      try {
        await this.$axios.$put(this.$config.api + "/products/" + this.$route.params.id, {
  ...
  async fetch() {
    ...
    try {
      ...
      this.form.vehicle_name = detailProduct.name,
        this.form.merk = detailProduct.merk,
        this.form.type = detailProduct.type,
        this.form.vehicle_category = detailProduct.category,
        this.form.location = detailProduct.location,
        this.imgTemp = detailProduct.image,
